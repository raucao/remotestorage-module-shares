	...
	RemoteStorage.defineModule('shares', function (privateClient, publicClient) {
	  ...
	  var shares = {
	    ...
	    storeFile: function storeFile(mimeType, name, data) {
	      ...
	      return publicClient.storeFile(mimeType, name, data).then(function () {
	    ...
	    _base64DecToArr: function _base64DecToArr(sBase64, nBlocksSize) {
	    ...
	    _isImage: function _isImage(type) {
	      return type.match(/(jpg|jpeg|png|gif|webp)$/i);
	    },

	    _createThumbnailFromBase64Image: function _createThumbnailFromBase64Image(base64, size) {
	      var img = new Image();
	      img.src = base64;
	      ...
	      octx.drawImage(img, sFrame.sX, sFrame.sY, sFrame.sW, sFrame.sH, 0, 0, oc.width, oc.height);
	      ...
	      ctx.drawImage(oc, 0, 0, iWidth, iHeight, 0, 0, canvas.width, canvas.height);
	    ...
	    _storeThumbnail: function _storeThumbnail(mimeType, name, data) {
